    ...
    def parse_string(self, nsstr, autostep, namespace=None):
        ...
        for opc, pat, rgnd in self._scan_string(nsstr, autostep):
            ...
            if self.group_resolver and pat[0] == '@':
                ...
                # here by directly matching the current (pattern, rangeset)
                # against the mask, but we would have to handle cases like
                # 'foo?' matches 'foo[1-9]' but NOT 'foo10'.
                # For now, we expand the just-parsed node masks and work with
                # this.
