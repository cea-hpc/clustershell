...
class EventHandler(object):
    ...
    Worker, EngineTimer or EnginePort selected events. If not implemented,
    ...
    def ev_pickup(self, worker, node):
        ...
        .. warning:: The signature of :meth:`EventHandler.ev_pickup` changed
            ...
            derived classes and add the node argument. DeprecationWarning is
            now being issued when using the old method's signature whose
            support will be removed in ClusterShell 2.0.
    ...
    def ev_read(self, worker, node, sname, msg):
        ...
        .. warning:: The signature of :meth:`EventHandler.ev_read` changed
            ...
            DeprecationWarning is now being issued when using the old method's
            signature whose support will be removed in ClusterShell 2.0.
    ...
    def ev_close(self, worker, did_timeout):
        ...
        .. warning:: The signature of :meth:`EventHandler.ev_close` changed
            in ClusterShell 1.8, please update your :class:`.EventHandler`
            derived classes to add the did_timeout argument. Please use this
            argument instead of the method ``ev_timeout``. DeprecationWarning
            is now being issued when using the old method's signature or the
            ``ev_timeout`` method, whose support will be removed in
            ClusterShell 2.0.
...
def _warn_signature(event_handler, event_name):
    """Warn for deprecated event method signature"""
    warnings.warn("%s: signature of %s() has changed: see pydoc ClusterShell.Event"
                  % (event_handler.__class__.__name__, event_name),
