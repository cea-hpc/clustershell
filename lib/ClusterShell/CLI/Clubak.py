...
def display_tree(tree, disp, out):
    ...
    for msgline, keys, depth, nchildren in tree.walk_trace():
        if togh:
            ...
            if line_mode:
                out.write(str(nodeset).encode('utf-8') + b':\n')
            else:
...
def clubak():
    ...
    separator = bytes(options.separator.encode('utf-8'))
    ...
    for line in sys_stdin():
        try:
            ...
            key = key.strip().decode('utf-8')  # NodeSet requires encoded string
            if not key:
            ...
            raise ValueError(str(ex) + ': "' + linestripped.decode('utf-8')
                             + '"')
