...
# RangeSet's backend type (v1.9+)
#  int: legacy, limited support for zero-padded ranges (compat)
#  str: v1.9+, support for mixed length zero-padded ranges (recommended)

# Default module backend type (constant)
# Useful to restore BACKEND_TYPE to its default value
DEFAULT_BACKEND_TYPE = int

# Used by new RangeSet objects when btype is not specified as an argument
# BACKEND_TYPE can be changed to str at run time for convenience
...
class RangeSet(set):
    ...
    RangeSet basic constructors:
       ...
       >>> rset = RangeSet()            # empty RangeSet default integer
                                        # backend type
       >>> rset = RangeSet("5,10-42")   # contains 5, 10 to 42
       >>> rset = RangeSet("0-10/2")    # contains 0, 2, 4, 6, 8, 10
       >>> rset = RangeSet("00-10/2")   # contains 0, 2, 4, 6, 8, 10

       >>> rset = RangeSet(btype=str)   # empty RangeSet with string backend
       >>> rset = RangeSet("5,10-42")   # contains '5', '10' to '42'
       >>> rset = RangeSet("0-10/2")    # contains '0', '2', '4', '6', '8', '10'
    ...
    Padding of ranges (eg. "003-009") is managed automatically and deducted from
    input arguments. This is new in ClusterShell v1.9, where mixed lengths zero
    padding is now supported within the same RangeSet. The instance variable
    padding has been deprecated and has no effect anymore.

    RangeSet is itself an iterator over its items as strings (since v1.9).
    ...
    def __init__(self, pattern=None, autostep=None, btype=None):
    ...
    @classmethod
    def fromone(cls, index, pad=0, autostep=None, btype=None):
        """
        ...
        """
        inst = RangeSet(autostep=autostep, btype=btype)
    ...
    def padding(self, value):
        if self.btype is str:
            raise TypeError("RangeSet.padding not supported with str backend")
        self._padding = value
    ...
    def __setstate__(self, dic):
        ...
        if getattr(self, '_version', 0) < RangeSet._VERSION:
            ...
            if getattr(self, '_version', 0) == 3:  # 1.6 - 1.8
                ...
                padding = getattr(self, 'padding', 0) or 0
                cpyset = set(self)
                self.clear()
                self.btype = str
                for i in cpyset:
    ...
    def slices2(self, autostep=AUTOSTEP_DISABLED):
        ...
        for si in self._sorted():
            ...
            if self.btype is int:
                assert type(si) is int
                idx, digitlen = si, self._padding
                # is current digit zero-padded? integer mode
                padded = (self._padding > 1)
            else:
                idx, digitlen = int(si), len(si)
                # is current digit zero-padded? string mode
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, slice):
            inst = RangeSet(btype=self.btype)
            inst._autostep = self._autostep
            inst._padding = self._padding
    ...
    def add_range(self, start, stop, step=1, pad=0):
        ...
        if pad is not None and pad > 0 and self._padding is None:
            self._padding = pad
        ...
        if self.btype == int:
            ...
            if pad == 0:
                ...
                set.update(self, ("%0*d" % (pad, i) for i in range(start, stop,
                                                                   step)))

    def copy(self):
        """Return a shallow copy of a RangeSet."""
        cpy = self.__class__(btype=self.btype)
    ...
    def __contains__(self, element):
        ...
        return set.__contains__(self, self.btype(element))
    ...
    def _binary_sanity_check(self, other):
        ...
        if self.btype is not other.btype:
            raise TypeError("Not supported between sets with different backend type")
        if not isinstance(other, set):
    ...
    def update(self, iterable):
        ...
        if isinstance(iterable, RangeSet):
            ...
            if self._padding is None and iterable.padding is not None:
                self._padding = iterable.padding
    ...
    def updaten(self, rangesets):
        ...
        for rng in rangesets:
            if isinstance(rng, set):
                self.update(self.btype(i) for i in rng)
            else:
                self.update(RangeSet(rng, btype=self.btype))
    ...
    def add(self, element, pad=0):
        ...
        if self.btype is str and not isinstance(element, str):
            set.add(self, "%0*d" % (pad, element))
        else:
            set.add(self, self.btype(element))
    ...
    def remove(self, element, pad=0):
        ...
        if self.btype is str and not isinstance(element, str):
            set.remove(self, "%0*d" % (pad, element))
        else:
            set.remove(self, self.btype(element))
    ...
    def discard(self, element, pad=0):
        ...
        try:
            if self.btype is str and not isinstance(element, str):
                set.discard(self, "%0*d" % (pad, element))
            else:
                set.discard(self, self.btype(element))
        except ValueError:
...
class RangeSetND(object):
            ...
            if rgvec:
                if isinstance(rgvec[0], str):
                    ...
                    if pads is None:
                        ...
                        self._veclist.append( \
                            [RangeSet.fromone(rg, pad, autostep, btype) \
    ...
    def pads(self):
        ...
        if self.btype is str:
            raise TypeError("pads() is not supported with str backend")
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, slice):
            ...
            if self.btype is int:
                ...
                pads = None
            rnd = RangeSetND(iveclist[index], pads=pads, autostep=self.autostep,
                             btype=self.btype)
            return rnd
    ...
    def intersection_update(self, other):
        ...
        empty_rset = RangeSet(btype=self.btype)
