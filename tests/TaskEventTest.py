...
class LegacyTestHandler(EventHandler):
...
class CS18TestHandler(EventHandler):
...
class CS18AbortOnReadHandler(EventHandler):
    def ev_read(self, worker):
        worker.abort()
...
class TaskEventTest(unittest.TestCase):
    ...
    def run_task_and_catch_warnings(self, task, expected_warn_cnt=0,
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            task.resume(task_timeout)
            self.assertEqual(len(w), expected_warn_cnt)
            if len(w) > 0:
    ...
    def test_simple_event_handler_legacy(self):
        ...
        self.run_task_and_catch_warnings(task, 3)  # pickup + read + close
    ...
    def test_simple_event_handler_with_task_timeout_legacy(self):
        ...
        try:
            # expected warnings: pickup + close
    ...
    def test_popen_specific_behaviour(self):
        ...
        class CS18WorkerPopenEH(CS18TestHandler):
            def __init__(self, testcase):
                CS18TestHandler.__init__(self)
                self.testcase = testcase
                self.worker = None

            def ev_start(self, worker):
                CS18TestHandler.ev_start(self, worker)
                self.testcase.assertEqual(worker, self.worker)

            def ev_read(self, worker, node, sname, msg):
                CS18TestHandler.ev_read(self, worker, node, sname, msg)
                self.testcase.assertEqual(worker, self.worker)
                self.testcase.assertEqual(worker, node)

            def ev_written(self, worker, node, sname, size):
                CS18TestHandler.ev_written(self, worker, node, sname, size)
                self.testcase.assertEqual(worker, self.worker)
                self.testcase.assertEqual(worker, node)

            def ev_pickup(self, worker, node):
                CS18TestHandler.ev_pickup(self, worker, node)
                self.testcase.assertEqual(worker, self.worker)
                self.testcase.assertEqual(worker, node)

            def ev_hup(self, worker, node, rc):
                CS18TestHandler.ev_hup(self, worker, node, rc)
                self.testcase.assertEqual(worker, self.worker)
                self.testcase.assertEqual(worker, node)

            def ev_close(self, worker, did_timeout):
                CS18TestHandler.ev_close(self, worker, did_timeout)
                self.testcase.assertEqual(worker.current_node, None)  # XXX

        task = task_self()
        eh = CS18WorkerPopenEH(self)
    ...
    def test_engine_on_the_fly_launch_legacy(self):
        ...
        # 1 x ev_pickup + 1 x ev_read + 2 x ev_pickup + 3 x ev_close
        self.run_task_and_catch_warnings(task, 7)

    class CS18TOnTheFlyLauncher(EventHandler):
    ...
    def test_engine_on_the_fly_launch(self):
        ...
        task = task_self()
        eh = self.__class__.CS18TOnTheFlyLauncher()
        worker = task.shell("/bin/uname", handler=eh)
        self.assertNotEqual(worker, None)
    ...
    def test_write_on_ev_start_legacy(self):
        ...
        self.run_task_and_catch_warnings(task, 1)  # read

    class CS18TWriteOnStart(EventHandler):
        def ev_start(self, worker):
            assert worker.task.running()
            worker.write(b"foo bar\n")
        def ev_read(self, worker, node, sname, msg):
    ...
    def test_write_on_ev_start(self):
        ...
        task = task_self()
        task.shell("cat", handler=self.__class__.CS18TWriteOnStart())
        self.run_task_and_catch_warnings(task)
    ...
    def test_engine_may_reuse_fd_legacy(self):
        ...
        try:
            ...
            for i in range(10):
                ...
                worker.write(b"OK\n")
            self.run_task_and_catch_warnings(task, 10)  # 10 x read
        finally:
    ...
    def test_engine_may_reuse_fd(self):
        ...
        try:
            task.set_info("fanout", 1)
            eh = self.__class__.CS18AbortOnReadHandler()
            for i in range(10):
                worker = task.shell("echo ok; sleep 1", handler=eh)
                self.assertTrue(worker is not None)
                worker.write(b"OK\n")
    ...
    def test_ev_pickup_legacy(self):
        ...
        self.run_task_and_catch_warnings(task, 6)  # 3 x pickup + 3 x close
