...
class MsgTreeTest(unittest.TestCase):
    ...
    def test_006_scalability(self):
        ...
        for i in range(10000):
            tree.add("node%d" % i, b"message" + str(i).encode('utf-8'))
        ...
        tree = MsgTree()
        for i in range(10000):
            tree.add("nodeX", b"message" + str(i).encode('utf-8'))
        self.assertEqual(len(tree), 1)
        cnt = 0
        for msg, keys in tree.walk():
        ...
        for j in range(100):
            for i in range(1000):
                tree.add("node%d" % j, b"message" + str(i).encode('utf-8'))
    ...
    def test_011_str_compat(self):
        ...
        if sys.version_info >= (3, 0):
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                # Trigger warning by using str() instead of bytes()
                s = str(elem)
                self.assertEqual(s, "message0")
                # Check warning
