...
class TreeWorkerTest(unittest.TestCase):
    ...
    def _tree_copy_dir(self, target):
        ...
        try:
            ...
            with open(join(copy_destdir, basename(file1.name)), 'rb') as rfile1:
                self.assertEqual(rfile1.read(), b'Lorem Ipsum Unum')
            with open(join(copy_destdir, basename(file2.name)), 'rb') as rfile2:
    ...
    def _tree_rcopy_dir(self, target, dirsuffix=None):
        ...
        try:
            ...
            with open(join(rcopy_destdir, basename(file1.name)), 'rb') as rfile1:
                self.assertEqual(rfile1.read(), b'Lorem Ipsum Unum')
            with open(join(rcopy_destdir, basename(file2.name)), 'rb') as rfile2:
                self.assertEqual(rfile2.read(), b'Lorem Ipsum Duo')
        finally:
            # src
            file1 = None
