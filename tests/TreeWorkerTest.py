...
This unit test requires working ssh connections to the following local
addresses:

    $HOSTNAME, localhost, 127.0.0.[2-4]

You can use the following options in ~/.ssh/config:

    Host your_hostname localhost 127.0.0.2 127.0.0.3 127.0.0.4
...
class TreeWorkerTest(unittest.TestCase):
    ...
    Connections are really established to the target and command results are
    tested.
    """
    ...
    def setUp(self):
        ...
        graph.add_route(NodeSet('localhost'), NodeSet(NODE_DISTANT))
    ...
    def test_tree_run_noremote(self):
        ...
        self.assertEqual(teh.ev_timedout_cnt, 0)
        self.assertEqual(teh.ev_close_cnt, 1)
        self.assertEqual(teh.last_read, b'127.0.0.2')
    ...
    def _tree_copy_file(self, target):
        teh = TEventHandler()
        srcf = make_temp_file('Lorem Ipsum', 'test_tree_copy_file_src')
    ...
    def _tree_copy_dir(self, target):
        ...
        try:
            ...
            # copy successful?
            copy_destdir = join(destdir, srcdir)
            with open(join(copy_destdir, basename(file1.name)), 'r') as rfile1:
                self.assertEqual(rfile1.read(), 'Lorem Ipsum Unum')
            with open(join(copy_destdir, basename(file2.name)), 'r') as rfile2:
                self.assertEqual(rfile2.read(), 'Lorem Ipsum Duo')
        finally:
            # src
            file1 = None
            file2 = None
    ...
    def _tree_rcopy_dir(self, target, dirsuffix=None):
        ...
        try:
            ...
            if not dirsuffix:
                dirsuffix = target
            rcopy_destdir = join(destdir, basename(srcdir) + '.' + dirsuffix)
            with open(join(rcopy_destdir, basename(file1.name)), 'r') as rfile1:
                self.assertEqual(rfile1.read(), 'Lorem Ipsum Unum')
            with open(join(rcopy_destdir, basename(file2.name)), 'r') as rfile2:
                self.assertEqual(rfile2.read(), 'Lorem Ipsum Duo')
        finally:
            # src
            file1 = None
    ...
    def test_tree_worker_bad_arguments(self):
        teh = TEventHandler()
