...
You can make the first dictionary found on this file (also named *cluster*) to
be the **default** source by changing ``default: local`` to ``default:
cluster`` in ``/etc/clustershell/groups.conf`` (main config file for groups).
...
The special group ``@*`` (or ``@source:*`` if using explicit source selection)
has been added if the group **all** exists in flat files or if the **all**
upcall is defined for external sources.
