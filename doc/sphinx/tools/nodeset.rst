...
class-NodeSet, and class-RangeSet). Most of the examples in this section are
using simple indexed node sets, however, *nodeset* supports multidimensional
node sets, like *dc[1-2]n[1-99]*, introduced in version 1.7 (see
class-RangeSetND for more info).
...
|                    | source* are listed (see groups-config for default   |
|                    | *group source* configuration).                      |
+--------------------+-----------------------------------------------------+
| ``--groupsources`` | List all configured *group sources*, one per line,  |
|                    | as configured in *groups.conf* (see groups-config). |
...
Zero-padding and stepping (as seen in nodeset-stepping) together are also
...
class-NodeSet-extended-patterns, there is an example of use::
...
is based on  :meth:`.NodeSet.groups` method, and should be specified with
...
applications (cf. groups-config). Having those group sources, group
provisioning is easily done through user-defined external shell commands.
Thus, node groups might be very dynamic and their nodes might change very
often. However, for performance reasons, external call results are still
cached in memory to avoid duplicate external calls during *nodeset*
execution. For example, a group source can be bound to a resource manager or a
custom cluster database.

For further details about using node groups in Python, please see
class-NodeSet-groups. For advanced usage, you should also be able to define
your own group source directly in Python (cf. class-NodeSet-groups-override).
...
If the **list** external shell command is configured (see groups-config, it is
possible to list available groups (from the default source) with the following
commands::
...
If needed group configuration conditions are met (cf. groups-config), you can
try group lookups thanks to the ``-r, --regroup`` command. This feature is
implemented with the help of the :meth:`.NodeSet.regroup()` method (see
class-NodeSet-regroup for further details). Only exact matching groups are
returned (all containing nodes needed), for example::
...
through its *extended pattern* feature (see class-NodeSet-extended-patterns).
...
Stepping and auto-stepping are supported (cf. nodeset-stepping) and also
zero-padding (cf. nodeset-zpad), which are both :class:`.RangeSet` class
features anyway.
