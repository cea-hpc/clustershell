...
The option ``--hostfile`` may be used to specify a path to a file containing a
list of single hosts, node sets or node groups, separated by spaces and lines.
It may be specified multiple times (one per file).
...
.. warning:: Host files are always parsed **before** any other command line
   options used for node selection or exclusion like ``-w`` and ``-x``.
