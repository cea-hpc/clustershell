...
class-NodeSet-extended-patterns) to benefits from :class:`.NodeSet` basic
arithmetics (like ``@Agroup&@Bgroup``). Additionally, the ``-x`` option allows
you to exclude nodes from remote hosts list (the same NodeSet syntax can be
used here). Nodes exclusion has priority over nodes addition.
...
:ref:`groups-config` for more details on special **all** external shell
command upcall configuration).  If not properly configured, the ``-a`` option
may lead to a runtime error like::
...
task-default-worker for default worker selection). ClusterShell supports other
worker types like *rsh* or also *pdsh*. This worker selection could be changed
at runtime thanks to ``--worker`` command line option::
