...
The \nodeset command enables easy manipulation of node sets, as well as node groups, at the command line level. As it is very user-friendly and efficient, the \nodeset command can quickly improve traditional cluster shell scripts. It is also full-featured as it provides most of the \NodeSet  and \RangeSet class methods (see section \ref{class-NodeSet} page \pageref{class-NodeSet} for \NodeSet, and \ref{class-RangeSet} page \pageref{class-RangeSet} for \RangeSet). Most of the examples in this section are using simple indexed node sets, however, \nodeset supports multidimensional node sets (eg. "\verb+dc[1-2]n[1-99]+") introduced in version 1.7 (see section~\ref{class-RangeSetND} page~\pageref{class-RangeSetND} for more info).
...
\hline
\verb+-e, --expand+ & Expand node sets or/and node groups as unitary node names separated by current separator string (see \verb+--separator+ option described in section~\ref{nodeset-commands-formatting} page~\pageref{nodeset-commands-formatting}).\\
\hline
...
Most commands described in this section produce output results that may be formatted using \verb+--output-format+ and \verb+--separator+ which are described in section~\ref{nodeset-commands-formatting} page~\pageref{nodeset-commands-formatting}.

\subsubsection{Commands with no input}
\label{nodeset-commands-noinput}
...
\subsubsection{Output result formatting}
\label{nodeset-commands-formatting}

When using the expand command \verb+-e, --expand+, a separator string is used when displaying results. The option \verb+--separator+ (or~\verb+-S+) allows you to modify it. The specified string is interpreted, so that you can use special characters as separator, like \verb+\n+ or \verb+\t+. The default separator is the space character\mbox{ " "}. This is an example showing such separator string change:
\medskip
\begin{lstlisting}[breaklines=true, breakatwhitespace=true]
$ nodeset -e --separator='\n' node[0-3]
node0
...
The option \verb+--output-format+ (or~\verb+-O+) can be used to format output results of most \nodeset commands. The string passed to this option is used as a base format pattern applied to each result. The default format string is\mbox{ "\%s"}. Formatting is performed using the Python builtin string formatting operator, so you must use one format operator of the right type (\verb+%s+ is guaranteed to work in all cases). A simple example when using the fold command is shown below:
\medskip
\begin{lstlisting}[breaklines=true, breakatwhitespace=true]
...
Output formatting and separator may be combined when using the expand command:
\medskip
\begin{lstlisting}[breaklines=true, breakatwhitespace=true]
$ nodeset -O '%s-ipmi' -S '\n' -e node[1-2]x[1-2]
node1x1-ipmi
...
\begin{itemize}
\item two modes of parallel cluster commands execution:
    \begin{itemize}
        \item flat mode: sliding window of local or \textit{ssh(1)} commands
        \item tree mode: commands propagated to the targets through a tree of pre-configured gateways; gateways are then using a sliding window of local or \textit{ssh(1)} commands to reach the targets (if the target count per gateway is greater than the fanout value)
    \end{itemize}
\item{smart display of command results (integrated output gathering, sorting by node, nodeset or node groups)}
\item{standard input redirection to remote nodes}
...
ClusterShell Tree mode is a major horizontal scalability improvement by enabling a hierarchical command propagation scheme. This section describes how to use the tree mode with the \clush command.

\subsubsection{Enabling tree mode}
\label{clush-tree-enabling}

As of version 1.7, the tree mode is enabled by default when a configuration is present. The system-wide library configuration file \verb+/etc/clustershell/topology.conf+ defines the routes of default command propagation tree. When this file exists, \clush will use it by default for target nodes that are defined there. This file path can be changed at runtime using the \verb+--topology+ command line option.

% how to disable tree mode ?

\subsubsection{Tree-related options}
\label{clush-tree-options}

The \verb+--remote=yes|no+ command line option controls the remote execution behavior:
\begin{itemize}
\item Default is \verb+yes+, that will make \clush establish connections up to the leaf nodes using a \emph{distant worker} like \textit{ssh}.
\item Changing it to \verb+no+ will make \clush establish connections up to the leaf parent nodes only, then the commands are executed locally on the gateways (like if it would be with \verb+--worker=exec+ on the gateways themselves). This execution mode allows users to schedule remote commands on gateways that take a node as an argument. On large clusters, this is useful to spread the load and resources used of one-shot monitoring, IPMI, or other commands on gateways. A simple example of use is:
    \begin{itemize}
    \item \lstinline+$ clush -w node[100-199] --remote=no /usr/sbin/ipmipower -h %h-ipmi -s+
    \end{itemize}
This command is also valid if you don't have any tree configured, because in that case, \verb+--remote=no+ is an alias of \verb+--worker=exec+ worker.
\end{itemize}

The \verb+--grooming+ command line option allows users to change the grooming delay (float, in seconds). This feature allows gateways to aggregate responses received within a certain timeframe before transmitting them back to the root node in a batch fashion. This contributes to reducing the load on the root node by delegating the first steps of this CPU intensive task to the gateways.
...
\hline
\verb+ssh_path+ & Set the \textit{ssh(1)} binary path to use for remote connection (default is \verb+ssh+).\\
\hline
\verb+ssh_options+ & Set additional (raw) options to pass to the underlying \textit{ssh(1)} command.\\
\hline
\verb+scp_path+ & Set the \textit{scp(1)} binary path to use for remote copy (default is \verb+scp+).\\
\hline
\verb+scp_options+ & Set additional options to pass to the underlying \textit{scp(1)} command. If not specified, \verb+ssh_options+ are used instead.\\
\hline
\verb+rsh_path+ & Set the \verb+rsh+ binary path to use for remote connection (default is
  \verb+rsh+). You could easily use \verb+mrsh+ or \verb+krsh+ by simply changing this value.\\
\hline
\verb+rcp_path+ & Same as \verb+rsh_path+ for rcp command (default is \verb+rcp+).\\
\hline
\verb+rsh_options+ & Set additional options to pass to the underlying rsh/rcp command.\\
\hline
\end{tabular}
\end{center}
...
\subsubsection{\RangeSetND class}
\label{class-RangeSetND}

The \RangeSetND class builds a N-dimensional RangeSet mutable object and provides the common set methods. This class is public and may be used directly, however we think it is less convenient to manipulate that \NodeSet and does not necessarily provide the same one-dimension optimization (see~\ref{class-NodeSet-nD} page~\pageref{class-NodeSet-nD}). Several constructors are available, using \RangeSet objects, strings or individual multidimensional tuples, for instance:
...
print rnd       # '0-5; 10-12\n'
print list(rnd) # [(0, 10), (0, 11), (0, 12), (1, 10), (1, 11), (1, 12), (2, 10), (2, 11), (2, 12), (3, 10), (3, 11), (3, 12), (4, 10), (4, 11), (4, 12), (5, 10), (5, 11), (5, 12)]


r1 = RangeSetND([(0, 4), (0, 5), (1, 4), (1, 5)])
print len(r)  # 4
print str(r1) # '0-1; 4-5\n'
r2 = RangeSetND([(1, 4), (1, 5), (1, 6), (2, 5)])
print str(r2) # '1; 4-6\n2; 5\n'
r = r1 & r2
print str(r)  # '1; 4-5\n'
print list(r) # [(1, 4), (1, 5)]

\end{lstlisting}

Please refer to the \RangeSetND API documentation for more information:
\begin{verbatim}
$ pydoc ClusterShell.RangeSet.RangeSetND
\end{verbatim}
...
\subsubsection{Getting \Task results}

After the task is finished (after \lstinline+Task.resume()+ or \lstinline+Task.run()+) or after a worker is completed when you have previously defined an event handler (at \lstinline+ev_close+), you can use \Task result getters:

\begin{lstlisting}[breaklines=true, breakatwhitespace=true]
task.iter_buffers([match_keys=nodelist])
task.iter_errors([match_keys=nodelist])
task.node_buffer(nodename)
task.node_error(nodename)
task.max_retcode()
task.num_timeout()
task.iter_keys_timeout()
\end{lstlisting}

\lstinline+buffer+ refers to standard output, \lstinline+error+ to standard error.

Please see some examples in section \ref{examples} or refer to the API documentation for more information:
\begin{verbatim}
...
When calling \lstinline+task.shell()+ or \lstinline+task.copy()+ the \Task object creates a worker instance for each call. When the \lstinline+nodes+ argument is defined, the worker class used for these calls is based on \Task default \verb+distant_worker+. Change this value to use another worker class, by example \lstinline+Rsh+:
\medskip
\begin{lstlisting}[breaklines=true, breakatwhitespace=true]
from ClusterShell.Task import task_self
...
The following example shows how to use the event-based programmation model by installing an EventHandler and listening for \lstinline+ev_read+ (we've got a line to read) and \lstinline+ev_hup+ (one command has just completed) events. The goal here is to print standard outputs of ``uname -a'' commands during their execution and also to notify the user of any erroneous return codes.
\medskip
\begin{lstlisting}[breaklines=true, breakatwhitespace=true]
from ClusterShell.Task import task_self
