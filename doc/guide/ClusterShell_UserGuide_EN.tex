...
\hline
\verb+fanout+& Size of the sliding window of \verb+ssh+ connectors.\\
\hline
\verb+connect_timeout+ & Timeout  in  seconds  to  allow a connection to establish. This parameter is passed to \verb+ssh+. If set to 0, no timeout occurs.\\
\hline
\verb+command_timeout+ & Timeout in seconds to allow a command to complete since the connection has been established. This  parameter is passed to \verb+ssh+.  In addition, the ClusterShell library ensures that any commands complete in less than (\verb+connect_timeout+ + \verb+command_timeout+). If set to 0, no timeout occurs.\\
\hline
...
\hline
\verb+ssh_user+ & Set the \verb+ssh+ user to use for remote connection (default is to not specify).\\
\hline
\verb+ssh_path+ & Set the \verb+ssh+ binary path to use for remote connection (default is \verb+/usr/bin/ssh+).\\
\hline
\verb+ssh_options+ & Set additional (raw) options to pass to the underlying \verb+ssh+ command.\\
\hline
\end{tabular}
\end{center}
...
Some tests assume that ssh to \verb+localhost+ is allowed for the current user.
...
If a Task does not have anymore scheduled worker or timer (for example, if you run one shell command and then it closes), it exits automatically from \lstinline+task.resume()+. Also, at any time (for example, in an event handler), you may call the following method to abort the Task execution:

\begin{lstlisting}[breaklines=true, breakatwhitespace=true]
task.abort()
\end{lstlisting}

On abort, all scheduled workers (shell command, file copy) and timers are cleaned and \lstinline+task.resume()+ returns, unblocking the Task's thread from a library user point of view.
